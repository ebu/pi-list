...
namespace
{
    ...
    const std::vector<video::colorimetry> available_colorimetry = {
        video::colorimetry::BT601,  video::colorimetry::BT709,  video::colorimetry::BT2020,
        video::colorimetry::BT2100, video::colorimetry::ST2063, video::colorimetry::ST2065,
        video::colorimetry::XYZ,    video::colorimetry::UNKNOWN};

    using scan_serializable                             = std::pair<std::string, video::scan_type>;
    const std::vector<scan_serializable> available_scan = {{"Progressive", video::scan_type::PROGRESSIVE},
    ...
    const std::vector<sp_rate> available_audio_sp_rate = {{"48kHz", audio::audio_sampling::_48kHz},
                                                          {"96kHz", audio::audio_sampling::_96kHz}};

    using ptime                                 = std::pair<std::string, audio_packet_time>;
    const std::vector<ptime> audio_packet_times = {{"125 microseconds", 125us},
    ...
    //----------------------------------------------------------------
    template <class ENUM> nlohmann::json to_json(ENUM this_enum)
    {
        const auto val = to_string(this_enum);
        return {{"label", val}, {"value", val}};
    }

    template <class T> nlohmann::json to_json(std::pair<std::string, T> t)
    {
