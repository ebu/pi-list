...
void analysis::write_frame_info(const path& base_dir, const std::string& stream_id, const frame_info& info)
{
    write_json_to(base_dir / stream_id / std::to_string(info.timestamp), constants::meta_filename,
                  frame_info::to_json(info));
}

void analysis::write_packets(const path& packets_path, const packets& packets_info)
{
    json j;
