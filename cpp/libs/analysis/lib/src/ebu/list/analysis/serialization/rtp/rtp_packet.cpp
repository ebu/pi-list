...
rtp_packet rtp_packet::from_json(const nlohmann::json& j)
{
    ...
    p.udp::datagram_info::destination_address =
        ipv4::from_dotted_string(j.at("destination_address").get<std::string>());
    ...
    p.udp::datagram_info::type           = ipv4::protocol_type::UDP; // todo: de-hardcode if we ever support more
                                                                     // protocols
