...
string media::to_string(media_type media_type)
{
    switch(media_type)
    {
        ...
        default:
            assert(media_type == media_type::UNKNOWN);
            return "unknown";
    }
}
...
{
    if( media == "video" ) return media_type::VIDEO;
    else if( media == "audio" ) return media_type::AUDIO;
    else if( media == "ancillary_data" ) return media_type::ANCILLARY_DATA;
    else return media_type::UNKNOWN;
}