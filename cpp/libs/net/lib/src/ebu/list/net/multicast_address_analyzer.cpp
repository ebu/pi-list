...
{
    const bool r = std::to_integer<uint8_t>(addr[0]) == 0x01 && std::to_integer<uint8_t>(addr[1]) == 0x00 &&
...
bool ebu_list::is_same_multicast_address(const ethernet::mac_address& mac_addr, const ipv4::address& ip_addr)
{
    ...
    const uint8_t ip_octet3 = static_cast<uint8_t>((le_addr & 0x00FF0000) >> 16);
    ...
    const bool r = ip_octet1 == mac_octet1 && ip_octet2 == mac_octet2 && (ip_octet3 & 0x7) == (mac_octet3 & 0x7);
    return r;
}
