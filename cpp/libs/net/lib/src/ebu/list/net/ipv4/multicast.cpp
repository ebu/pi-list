...
void ipv4::join_multicast_group(socket_handle& sock, ipv4::address listen_address, ipv4::endpoint multicast_endpoint)
{
    // Enable SO_REUSEADDR to allow multiple applications to receive copies of the multicast datagrams.
    int reuse = 1;
    const auto setsockopt_result =
        setsockopt(sock.get_handle(), SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&reuse), sizeof(reuse));
    ...
    const auto bind_result =
        bind(sock.get_handle(), reinterpret_cast<struct sockaddr*>(&localSockAddr), sizeof(localSockAddr));
