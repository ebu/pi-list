...
SCENARIO("srt data packet header")
{
    GIVEN("a buffer with 16 bytes")
    {
        ...
        WHEN("we check the fields of the first line in payload header")
        {
            ...
            THEN("packet type flag must be 1") { REQUIRE(packet_type_flag == 0x01); }
            THEN("sequence number must be 0xFE000000") { REQUIRE(sequence_number == 0x7F000000); }
        }
    }
    ...
    GIVEN("a buffer with 16 bytes")
    {
        ...
        WHEN("we check the fields of the second line in payload header")
        {
            ...
            THEN("packet position flag must be 11") { REQUIRE(packet_position_flag == 0x03); }
            THEN("order flag must be 1") { REQUIRE(order_flag == 0x01); }
            THEN("encryption flag must be 11") { REQUIRE(encryption_flag == 0x03); }
            THEN("retransmission flag must be 1") { REQUIRE(retransmission_flag == 0x01); }
            THEN("message number must be 0x03FF0000") { REQUIRE(message_number == 0x03FF0000); }
        }
    }
    ...
    GIVEN("a buffer with 16 bytes")
    {
        ...
        WHEN("we check the fields of the third and fourth line in payload header")
        {
            ...
            THEN("timestamp counter must be 0xFFFF00FF") { REQUIRE(timestamp == 0xFFFF00FF); }
            THEN("destination socket id must be 0xFF00FFFF") { REQUIRE(destination_socket_id == 0xFF00FFFF); }
        }
    }
    ...
    GIVEN("a buffer with 16 bytes")
    {
        ...
        WHEN("we check the fields of the payload header")
        {
            ...
            THEN("packet type flag must be 0") { REQUIRE(packet_type_flag == 0); }
            THEN("sequence number must be 105") { REQUIRE(sequence_number == 0x00000069); }

            THEN("packet position flag must be 11") { REQUIRE(packet_position_flag == 0x03); }
            THEN("order flag must be 0") { REQUIRE(order_flag == 0); }
            THEN("encryption flag must be 00") { REQUIRE(encryption_flag == 0); }
            THEN("retransmission flag must be 0") { REQUIRE(retransmission_flag == 0); }
            THEN("message number must be 105") { REQUIRE(message_number == 0x00000069); }
