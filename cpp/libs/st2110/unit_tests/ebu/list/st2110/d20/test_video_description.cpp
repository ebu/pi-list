...
SCENARIO("sdp creation for st2110 video")
{
    GIVEN("a st2110-20 video information")
    {
        ...
        WHEN("we generate the additional attributes tag")
        {
            ...
            {
                const std::vector<std::string> expected = { "a=fmtp:96 sampling=YCbCr-4:2:2; width=1920; height=1080; exactframerate=50; depth=10; colorimetry=BT601; PM=2110GPM; SSN=ST2110-20:2017" };
        ...
        WHEN("we generate the additional attributes tag with a not integer framerate")
        {
            ...
            THEN("we get the correct information")
            {
                const std::vector<std::string> expected = { "a=fmtp:96 sampling=YCbCr-4:2:2; width=1920; height=1080; exactframerate=60/1001; depth=10; colorimetry=BT601; PM=2110GPM; SSN=ST2110-20:2017" };
