...
int d21::calculate_cmax_narrow(read_schedule schedule, int n_packets, fraction t_frame, media::video::scan_type scan,
...
int d21::calculate_cmax_wide(int n_packets, fraction t_frame)
{
...
compliance_parameters d21::calculate_compliance_parameters(compliance_profile profile, int n_packets, fraction t_frame,
                                                           ...
                                                           media::video::video_dimensions raster)
{
    ...
    LIST_ENFORCE(profile == compliance_profile::wide, std::runtime_error, "Invalid compliance profile");
    return calculate_wide_compliance_parameters(n_packets, t_frame);
}

compliance_parameters d21::calculate_narrow_compliance_parameters(int n_packets, fraction t_frame,
                                                                  read_schedule schedule, media::video::scan_type scan,
                                                                  media::video::video_dimensions raster)
{
    const auto vrx_full = calculate_vrx_full_narrow(n_packets, t_frame);

    const auto c_max = calculate_cmax_narrow(schedule, n_packets, t_frame, scan, raster);

    return {vrx_full, c_max};
}

compliance_parameters d21::calculate_wide_compliance_parameters(int n_packets, fraction t_frame)
{
...
fraction64 st2110::d21::get_tro_default_multiplier(media::video::scan_type scan, media::video::video_dimensions raster)
{
    // ST2110-21, 6.3.2 Gapped PRS Progressive Images
    // ST2110-21, 6.3.3 Gapped PRS Interlaced and PsF Images
...
fraction64 st2110::d21::get_tro_default(fraction64 tframe, media::video::scan_type scan,
                                        media::video::video_dimensions raster)
{
...
vrx_constants st2110::d21::calculate_vrx_constants(int npackets, fraction64 tframe, read_schedule schedule,
                                                   media::video::scan_type scan, media::video::video_dimensions raster)
{
