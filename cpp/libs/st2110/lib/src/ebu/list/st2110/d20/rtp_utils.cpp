...
int64_t d20::calculate_n(fraction64 packet_time, fraction64 frame_period)
{
    ...
    const auto n0 =
        static_cast<double>(packet_time) / static_cast<double>(frame_period);
    return static_cast<int64_t>(std::floor(n0));
}

delta_info d20::calculate_rtp_to_packet_deltas(fraction64 frame_period,
                                               ...
                                               fraction64 packet_time)
{
    ...
    info.delta_rtp_vs_packet_time =
        to_ticks32(static_cast<int>(rtp_timestamp) -
                   static_cast<int>(rtp_ts_for_packet_time));
    info.delta_rtp_vs_NTs = to_ticks32(static_cast<int>(rtp_timestamp) -
                                       static_cast<int>(rtp_ts_for_NTs));
