...
st2110_30_sdp_serializer::st2110_30_sdp_serializer(const d30::audio_description& audio_des) : audio_desc_(audio_des)
{
}

void st2110_30_sdp_serializer::additional_attributes(std::vector<std::string>& current_lines,
...
void st2110_30_sdp_serializer::write_rtpmap_line(std::vector<std::string>& current_lines,
                                                 const ebu_list::media::network_media_description& media_description)
{
    /** a=rtpmap:<payload_type> <encoding>/<clock_rate>/<channels> **/
    current_lines.emplace_back(fmt::format("a=rtpmap:{} {}/{}/{}", media_description.network.payload_type,
                                           to_string(audio_desc_.encoding), to_int(audio_desc_.sampling),
                                           audio_desc_.number_channels));
}
