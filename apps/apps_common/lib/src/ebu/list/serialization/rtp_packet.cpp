#include "ebu/list/serialization/rtp_packet.h"
...
nlohmann::json rtp_packet::to_json(const rtp_packet& packet)
{
    ...
    j["short_sequence_number"] = packet.sequence_number; //todo: change to sequence number ?
    j["ssrc"] = packet.ssrc;

    // UDP stuff
    j["packet_time"] = packet.packet_time.time_since_epoch().count();
    j["destination_address"] = to_string(packet.destination_address);
    j["destination_port"] = to_string(packet.destination_port);
    j["source_address"] = to_string(packet.source_address);
    j["source_port"] = to_string(packet.source_port);
...
rtp_packet rtp_packet::from_json(const nlohmann::json& j)
{
    ...
    p.sequence_number = j.at("short_sequence_number").get<uint16_t>();
