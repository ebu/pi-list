...
namespace
{
    ...
    {
        const auto line_size = desc.dimensions.width *
                               video::samples_per_pixel(desc.sampling) *
                               desc.color_depth;
    ...
    int get_buffer_size_for(const video_description& desc)
    {
        const auto frame_height =
            desc.scan_type == media::video::scan_type::PROGRESSIVE
    ...
    void throw_if_inconsistent(const line_header_lens& header,
                               media::video::scan_type scan)
    {
        if (header.field_identification() != 0 &&
            scan != media::video::scan_type::INTERLACED)
        {
    ...
    void
    update_net_info_with_address_validation(media::network_info& info,
                                            const rtp::packet_info& packet_info)
    {
        info.valid_multicast_mac_address =
            info.valid_multicast_mac_address &&
            is_multicast_address(packet_info.ethernet_info.destination_address);

        info.valid_multicast_ip_address =
            info.valid_multicast_ip_address &&
            is_multicast_address(packet_info.udp.destination_address);

        info.multicast_address_match =
            info.multicast_address_match &&
            is_same_multicast_address(
                packet_info.ethernet_info.destination_address,
                packet_info.udp.destination_address);

//        logger()->trace(
//            "valid_multicast_mac_address: {} - valid_multicast_ip_address: {} - "
...
video_stream_handler::video_stream_handler(decode_video should_decode_video,
    ...
    : should_decode_video_(should_decode_video == decode_video::yes),
      info_(std::move(info)), video_description_(std::move(details)),
      completion_handler_(std::move(ch))
{
    ...
    video_description_.first_frame_ts =
        first_packet.info.rtp.view().timestamp();
...
void video_stream_handler::new_frame()
{
    ...
    {
        current_frame_->buffer = block_factory_.get_buffer(
            get_buffer_size_for(video_description_.video));
    }
...
{
    video_description_.video.rate =
        rate_.rate().value_or(media::video::Rate(0));
...
void video_stream_handler::on_complete()
{
    ...
    {
        video_description_.dropped_packet_count +=
            rtp_seqnum_analyzer_.dropped_packets();
        logger()->info("video rtp packet drop: {}",
                       video_description_.dropped_packet_count);
    }
...
void video_stream_handler::parse_packet(const rtp::packet& packet)
{
    ...
    packet_info info{
        packet.info, packet, *current_frame_, full_sequence_number, {}};
    ...
    if (should_decode_video_)
    {
        ...
        logger()->info("UDP packet size: {} SDU size: {}",
                       packet.info.udp.datagram_size, sdu.view().size_bytes());
        ...
        while (p < end)
        {
            const auto line_header =
                line_header_lens(*reinterpret_cast<const raw_line_header*>(p));
            p += sizeof(raw_line_header);

            throw_if_inconsistent(line_header,
                                  video_description_.video.scan_type);

            video_description_.max_line_number =
                std::max(video_description_.max_line_number,
                         int(line_header.line_number()));

            LIST_ENFORCE(line_index < info.line_info.size(), std::runtime_error,
                         "Only three lines per packet allowed by ST2110-20");

            info.line_info[line_index].continuation =
                line_header.continuation();
            info.line_info[line_index].field_identification =
                line_header.field_identification();
...
#if defined(LIST_TRACE)
            logger()->info(
                "Line in packet: {} Line no: {} Offset: {} Length: {}",
                line_index, info.line_info[line_index].line_number,
                info.line_info[line_index].offset,
        ...
        for (const auto& line : info.line_info)
        {
            ...
            {
                logger()->error(
                    "buffer out of bounds. Line no: {} Offset: {} Length: {}",
                    line.line_number, line.offset, line.length);
                break;
            }

            const auto byte_offset =
                offset_to_byte_offset(line.offset, video_description_.video);

            const auto target = current_frame_->buffer->begin() +
                                get_line_size_bytes(video_description_.video) *
                                    line.line_number +
                                byte_offset;
            ...
            byte_span target_data(target,
                                  current_frame_->buffer->end() - target);
