...
{
    constexpr uint32_t be_multicast_mask = 0x00000007;
    const uint32_t be_addr = static_cast<uint32_t>(addr);
    const bool r = (be_addr & be_multicast_mask) == be_multicast_mask;
    return r;
}
                                          ...
                                          const ipv4::address &ip_addr)
{
    ...
    const bool r = ip_octet1 == mac_octet1 &&
                   ip_octet2 == mac_octet2 &&
