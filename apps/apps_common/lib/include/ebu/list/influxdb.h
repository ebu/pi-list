...
namespace ebu_list::influx
{
    ...
    {
    public:
        caching_influx_writer(std::string_view url);
    ...
    public:
        ...
        template<typename T>
        void send_data(const std::string& key, T value, clock::time_point when)
        {
            const auto time = std::chrono::duration_cast<clock::duration >(when.time_since_epoch()).count();
            writer_.write(
                influxdb::api::line(pcap_id_, influxdb::api::key_value_pairs("stream", stream_id_), influxdb::api::key_value_pairs(key, value), details::packet_time_wrapper{ time })
            );
        }

        template<typename T>
        void send_data(const std::string& key, std::optional<T> value, clock::time_point when)
        {
