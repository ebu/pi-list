...
namespace ebu_list
{
    template<class ListenerT, class MessageT>
    struct multi_listener_t : public ListenerT
    {
        using listener_uptr = std::unique_ptr<ListenerT>;

    public:
        void add(listener_uptr&& sublistener)
        {
            listeners_.push_back(std::move(sublistener));
        }
