...
function AVSyncView({ comparisonInfo, mainStreamInfo, referenceStreamInfo }: any) {
    ...
    const onVideoChange = (frame: any, index: number) => {
        getVideoCursor(frame, index, video).then(e => {
            setVideoCursor(e);
        });
    };

    const onAudioChange = (mp3Duration: number, mp3CurrentTime: number) => {
        getAudioCursor(mp3Duration, mp3CurrentTime, audio).then(e => {
            setAudioCursor(e);
        });
    };
        ...
        <div>
            ...
            <div className="st20227-data-container">
                ...
                */
                ...
                <div>
                    ...
                    <span className="comparison-configuration-panel-data">{`${(delay.pkt / 1000).toFixed(3)}`}</span>
                    <span className="comparison-configuration-panel-data">
                        {' '}
                ...
                <div>
                    ...
                    <span className="comparison-configuration-panel-data">{`${(delay.rtp / 1000).toFixed(3)}`}</span>
            ...
            <div>
                <ImageGalleryStreamExplorer
                    currentStream={video}
                    pcapID={videoPcapId}
                    cursorInitPos={comparisonInfo.result.videoCursor.position}
                    onChange={onVideoChange}
                />
                <AudioPlayerDisplay
                    currentStream={audio}
                    pcapID={audioPcapId}
                    cursorInitPos={comparisonInfo.result.audioCursor.position}
