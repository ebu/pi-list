...
const sendMqttUpdate = (payload) =>
    mqttSender.send({
        key: api.mq.exchanges.mqtt.topics.nmos.sender_list_update,
        msg: payload,
    });
...
function updateLiveSource(_source) {
    ...
    const upsertedSource = LiveSource.findOneAndUpdate(filter, source, {
        new: false,
        overwrite: true
    }).exec();
...
async function mapAddedNmosToSource(sender) {
    ...
    try {
        ...
        if (sender.manifest_href === undefined) {
            logger('live-sources').error(`No manifest_href for sender with id ${sender.id}`);
            return source;
        }
...
const onChanged = async ({
    ...
    const addedSourcesPromises = added.map(async (a) => await mapAddedNmosToSource(a));
