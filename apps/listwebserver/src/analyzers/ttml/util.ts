...
// sync
const doLoadTtmlDocuments = (ttmlFilesPath: any, stream: any) => {
    const options = {
        cwd: ttmlFilesPath,
    };
    const xml_files = glob.sync('*', options);

    const xml_data = xml_files
        .map((f: any) => {
...
// sync
const doAnalyseTimeBase = (stream: any) => {
    const isAnyTimebaseNotMedia = stream.media_specific.data.some((d: any) => {
...
// sync
const doAnalyseSequenceIdentifier = (stream: any) => {
    const seqIds = [...new Set(stream.media_specific.data.map((d: any) => d.sequenceIdentifier))];
