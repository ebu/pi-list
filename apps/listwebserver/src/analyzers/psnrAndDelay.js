...
const getPsnrList = async (mainDirList, refDirList) => {
    const getPsnrPromises = mainDirList.map(async (path, index) => getPnsr(path, refDirList[index]));
    return Promise.all(getPsnrPromises);
};

const getPsnrMax = async (psnrList) => {
    const max = await psnrList.reduce(
        (max, e) => (max === 'inf' ? max : e === 'inf' ? e : e > max ? e : max),
        psnrList[0]
    );
...
const createComparator = async (config) => {
    ...
    logger('psnr-delay').info(
        `Compare ${mainDirList.length} frames of main sequence with frame #${refDirIndex} of reference sequence.`
    );
    ...
    logger('psnr-delay').info(
        `Compare ${mainDirList.length} frames of main sequence with ${refDirList.length} frames of shifted reference sequence, one to one.`
    );
