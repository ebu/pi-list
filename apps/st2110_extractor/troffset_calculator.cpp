...
{
  public:
    troffset_analyzer(std::string stream_id, media::video::info video_info,
                      tro_map& tro_info);
...
troffset_analyzer::troffset_analyzer(std::string stream_id,
    ...
    : stream_id_(stream_id), tro_info_(tro_info), t_frame_(1 / video_info.rate),
      tro_default_(st2110::d21::get_tro_default(
          1 / video_info.rate, video_info.scan, video_info.raster))
{}
...
void troffset_analyzer::on_complete()
{
    ...
    {
        tro_info_.insert(
            {stream_id_, tro_stream_info{to_ns(tro_default_), 0, 0, 0}});
    }
    else
    {
        const auto total_ns =
            std::accumulate(tros_.begin(), tros_.end(), int64_t{0});
...
tro_map ebu_list::calculate_average_troffset(
    ebu_list::path pcap_file, std::vector<stream_with_details> wanted_streams)
{
    tro_map tro_info;
    ...
    auto create_handler = [&](rtp::packet first_packet) -> rtp::listener_uptr {
        ...
        if (stream_info.type == media::media_type::VIDEO)
        {
            ...
            auto handler = std::make_unique<troffset_analyzer>(
                stream_info.id, video_info, tro_info);
    ...
    auto player =
        std::make_unique<pcap::pcap_player>(pcap_file, handler, on_error_exit);
