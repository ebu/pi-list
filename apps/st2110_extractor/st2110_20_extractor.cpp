...
namespace
{
    ...
    config parse_or_usage_and_exit(int argc, char const* const* argv)
    {
        ...
        const auto [parse_result, config] =
            parse(argc, argv,
                  ...
                  argument(&config::pcap_id, "pcap id",
                           "the pcap id to be processed"),
                  argument(&config::pcap_file, "pcap file",
                           "the path to the pcap file within the filesystem"),
        ...
        logger()->error("usage: {} {}", path(argv[0]).filename().string(),
                        to_string(parse_result));
        exit(-1);
    }

    ptp::state_machine_listener_ptr
    get_ptp_influx_logger(std::optional<std::string> influxdb_url,
                          std::string pcap_uuid)
    {
        if (influxdb_url)
        {
            return std::make_shared<influx::influxdb_ptp_logger>(
                influxdb_url.value_or(INFLUX_DEFAULT_URL), pcap_uuid);
        }
        else
        {
    ...
    std::vector<stream_with_details> get_ids_to_process(const db_serializer& db,
                                                        const config& config)
    {
        const auto look_for = nlohmann::json{{"pcap", config.pcap_id}};
        const auto r =
            db.find_many(constants::db::offline,
                         constants::db::collections::streams, look_for);

        std::vector<stream_with_details> found_streams;
        std::transform(r.begin(), r.end(), std::back_inserter(found_streams),
                       [](const auto& j) {
        ...
        const auto wanted_stream_it = std::find_if(
            begin(found_streams), end(found_streams), [&](const auto& info) {
                const auto& id = info.first.id;
                return id == config.id_to_process.value();
            });

        LIST_ENFORCE(wanted_stream_it != found_streams.end(),
                     std::runtime_error, "Didn't find specified stream: {}",
                     config.id_to_process.value());
        return {*wanted_stream_it};
    }

    void update_one_tr_info(db_serializer& db, std::string stream_id,
                            const tro_stream_info& info)
    {
        const auto stream_to_update = nlohmann::json{{"id", stream_id}};

        auto maybe_record =
            db.find_one(constants::db::offline,
                        constants::db::collections::streams, stream_to_update);
        ...
        db.update(constants::db::offline, constants::db::collections::streams,
                  stream_to_update, record);
    }
    ...
    template <class T>
    nlohmann::json gather_info(const serializable_stream_info& info,
                               const T& details)
    {
    ...
    void run(logger_ptr logger, const config& config)
    {
        ...
        const auto tro_info =
            calculate_average_troffset(config.pcap_file, wanted_streams);

        const auto look_for = nlohmann::json{{"id", config.pcap_id}};
        const auto result =
            db.find_one(constants::db::offline,
                        constants::db::collections::pcaps, look_for);
        LIST_ENFORCE(result, std::runtime_error, "Can't find pcap {} on DB",
                     config.pcap_id);
        auto pcap = pcap_info::from_json(result.value());

        const auto offset_from_ptp_clock = use_offset_from_ptp_clock
                                               ? -pcap.offset_from_ptp_clock
        ...
        auto video_dump_handler = [&](const video_stream_serializer& handler) {
            ...
            {
            case compliance_profile::narrow:
                nr_narrow++;
                break;
            case compliance_profile::narrow_linear:
                nr_narrow_linear++;
                break;
            case compliance_profile::wide:
                nr_wide++;
                break;
            case compliance_profile::not_compliant:
                nr_not_compliant++;
                break;
            }
            ...
            const auto stream_to_update =
                nlohmann::json{{"id", network_info.id}};
            ...
            db.update(constants::db::offline,
                      constants::db::collections::streams, stream_to_update, j);

            st2110::d20::st2110_20_sdp_serializer s(handler.info().video);
            ebu_list::sdp::sdp_writer sdp(
                {"LIST Generated SDP", "Video flow",
                 config.storage_folder / network_info.id / "video.sdp"});
        ...
        auto audio_dump_handler = [&](const audio_stream_handler& handler) {
            ...
            ebu_list::sdp::sdp_writer sdp(
                {"LIST Generated SDP", "audio flow",
                 config.storage_folder / network_info.id / "audio.sdp"});
            sdp.add_media(network_info, s);
            sdp.write();
        };
        ...
        auto anc_dump_handler = [&](const anc_stream_handler& handler) {
            ...
            ebu_list::sdp::sdp_writer sdp(
                {"LIST Generated SDP", "Ancillary flow",
                 config.storage_folder / network_info.id / "ancillary.sdp"});
            sdp.add_media(network_info, s);
            sdp.write();
        };

        auto create_handler =
            [&](rtp::packet first_packet) -> rtp::listener_uptr {
            nr_total++;
            const auto ssrc = first_packet.info.rtp.view().ssrc();
            const ipv4::endpoint source = {first_packet.info.udp.source_address,
                                           first_packet.info.udp.source_port};
            const ipv4::endpoint destination = {
                first_packet.info.udp.destination_address,
                first_packet.info.udp.destination_port};

            const auto stream_info_it =
                std::find_if(wanted_streams.begin(), wanted_streams.end(),
                             [&](const stream_with_details& details) {
                                 const auto& s = details.first;
                                 return s.network.ssrc == ssrc &&
                                        s.network.source == source &&
                                        s.network.destination == destination;
                             });
            ...
            {
                nr_video++;
                const auto& in_video_info =
                    std::get<video_stream_details>(stream_info_it->second);
                const auto video_info = media::video::info{
                    in_video_info.video.rate, in_video_info.video.scan_type,
                    in_video_info.video.dimensions};

                auto new_handler = std::make_unique<video_stream_serializer>(
                    first_packet, stream_info, in_video_info,
                    config.storage_folder, main_executor, video_dump_handler);
                auto ml = std::make_unique<
                    multi_listener_t<rtp::listener, rtp::packet>>();
                ...
                if (config.influxdb_url)
                {
                    ...
                    const auto influx_db_url =
                        config.influxdb_url.value_or(INFLUX_DEFAULT_URL);

                    {
                        auto db_logger =
                            std::make_unique<influx::influxdb_c_inst_logger>(
                                influx_db_url, pcap.id, stream_info.id);
                        auto cinst_writer = std::make_unique<histogram_writer>(
                            info_path, cinst_file_name);
                        auto analyzer = std::make_unique<c_analyzer>(
                            std::move(db_logger), std::move(cinst_writer),
                            in_video_info.video.packets_per_frame,
                            video_info.rate);
                        ml->add(std::move(analyzer));
                    }

                    {
                        auto framer_ml = std::make_unique<multi_listener_t<
                            frame_start_filter::listener,
                            frame_start_filter::packet_info>>();

                        {
                            auto db_logger = std::make_unique<
                                influx::influxdb_rtp_ts_logger>(
                                influx_db_url, pcap.id, stream_info.id);
                            auto analyzer = std::make_unique<rtp_ts_analyzer>(
                                std::move(db_logger), video_info.rate);
                            framer_ml->add(std::move(analyzer));
                        }

                        {
                            auto db_logger =
                                std::make_unique<influx::influxdb_vrx_logger>(
                                    influx_db_url, pcap.id, stream_info.id,
                                    "gapped-ideal");
                            auto vrx_writer =
                                std::make_unique<histogram_writer>(
                                    info_path, vrx_file_name);
                            const auto settings =
                                vrx_settings{in_video_info.video.schedule,
                                             tvd_kind::ideal, std::nullopt};
                            auto analyzer = std::make_unique<vrx_analyzer>(
                                std::move(db_logger), std::move(vrx_writer),
                                in_video_info.video.packets_per_frame,
                                video_info, settings);
                            framer_ml->add(std::move(analyzer));
                        }

                        {
                            ...
                            const auto tro = it == tro_info.end()
                                                 ? 0
                                                 : it->second.avg_tro_ns;
                            const auto settings = vrx_settings{
                                in_video_info.video.schedule, tvd_kind::ideal,
                                std::chrono::nanoseconds(tro)};
                            auto analyzer = std::make_unique<vrx_analyzer>(
                                std::move(db_logger), histogram_listener_uptr(),
                                in_video_info.video.packets_per_frame,
                                video_info, settings);
                            framer_ml->add(std::move(analyzer));
                        }

                        auto framer = std::make_unique<frame_start_filter>(
                            frame_start_filter::listener_uptr(
                                std::move(framer_ml)));
            ...
            else if (stream_info.type == media::media_type::AUDIO)
            {
                ...
                const auto& audio_info =
                    std::get<audio_stream_details>(stream_info_it->second);
                auto new_handler = std::make_unique<audio_stream_serializer>(
                    first_packet, stream_info, audio_info, audio_dump_handler,
            ...
            else if (stream_info.type == media::media_type::ANCILLARY_DATA)
            {
                ...
                const auto& anc_info =
                    std::get<anc_stream_details>(stream_info_it->second);
                auto new_handler = std::make_unique<anc_stream_serializer>(
                    first_packet, stream_info, anc_info, anc_dump_handler,
                    config.storage_folder);
                return new_handler;
            }
            else
            {
                logger->warn("Bypassing stream with ssrc: {}. Reason: Unknown "
                             "media type",
                             ssrc);
                logger->warn("\tdestination: {}", to_string(destination));
                auto handler = std::make_unique<rtp::null_listener>();
        ...
        auto player = std::make_unique<pcap::pcap_player>(
            config.pcap_file, filter, on_error_exit, offset_from_ptp_clock);
        ...
        db.update(constants::db::offline, constants::db::collections::pcaps,
                  look_for, pcap_info::to_json(pcap));
    }
} // namespace
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        const auto processing_time_ms = static_cast<double>(
            std::chrono::duration_cast<std::chrono::milliseconds>(
                processing_time)
                .count());
        logger()->info("Processing time: {:.3f} s",
